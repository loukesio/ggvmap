...
- **Pure R only.** No compiled code and no new hard dependencies —
  `ggplot2` is the only Import. Optional niceties go in `Suggests` behind
  `requireNamespace()` guards.
- **`README.md` is generated.** Edit `README.Rmd` only, then run
  `devtools::build_readme()`. The pre-rendered figures (hero, shapes grid,
...
- The geometric invariants in `tests/testthat/test-correctness.R` are the
  package's contract — never weaken them.
